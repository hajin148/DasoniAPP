                ...
                mFirebaseAuth.signInWithEmailAndPassword(strEmail, strPwd).addOnCompleteListener(LoginActivity.this, new OnCompleteListener<AuthResult>() {
                    ...
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        if (task.isSuccessful()) {
                            ...
                            // Get reference to the user's data in Firebase
                            DatabaseReference userRef = FirebaseDatabase.getInstance().getReference("DasoniAPP/users").child(userId);

                            // Fetch the user's current data and update login times
                            userRef.addListenerForSingleValueEvent(new ValueEventListener() {
                                @Override
                                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                    UserAccount userAccount = dataSnapshot.getValue(UserAccount.class);
                                    if (userAccount != null) {
                                        userAccount.setDateAccess();
                                        userAccount.setTimeFirstAccess();
                                        userRef.setValue(userAccount);
                                    }
